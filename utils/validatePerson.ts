...
export const validatePerson = (person: any): [boolean, string[]] => {
  ...
  if (person.year && !Object.keys(YearObject)
    .includes(person.year.toString())) {
    errors.push(`valid years are: ${Object.keys(YearObject)}`);
  }

  if (!Object.keys(PersonTypeObject)
    .includes(person.type)) {
    errors.push(`valid types are: ${Object.keys(PersonTypeObject)}`);
  }

  if (person.society && !Object.keys(SocietyObject)
    .includes(person.society)) {
