...
const EditPersonScoreListItem: React.FC<PersonListItemProps> = ({ person }) => {
  ...
  const color = (() => {
    ...
    return changeType === '+' ? 'positive' : 'negative';
  })();

  const listItem = (
    <StyledTrigger aria-label={`${name} ${family}, ${statusCollection[(status as Status)]}`}>
      <StyledDiv style={{ width: '.8rem', textAlign: 'center' }}>
        {romanNumbers[year]}
      </StyledDiv>
      ...
      <StyledDiv $color={color} style={{ justifySelf: 'flex-end' }}>
        {changeType === '+' || scoreChange < 0 ? score + scoreChange : score - scoreChange}
      </StyledDiv>
    </StyledTrigger>
  );
  ...
  return (
    <StyledListItem $status={status}>
      <Container style={{ justifySelf: 'left', display: 'flex', justifyContent: 'center' }}>
        <PersonModal
          trigger={listItem}
          personId={id ?? ''}
        />
      ...
      <ScoreCounter
        changeType={scoreChange < 0 ? '-' : changeType}
        name={id?.toString()}
