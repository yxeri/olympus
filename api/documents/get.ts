...
export const hasAccessToDocument = ({
  ...
  || [
    ...
    ...((hasPostAccess ? document.postAccess : document.readAccess) ?? [])
...
export const findDocument: ({
  ...
  _id: ObjectId,
  authPerson?: Person,
}) => Promise<Document | null> = async ({ _id, authPerson }) => {
  ...
  const document = await documentCollection.findOne({ _id, $or: orFilter });

  if (document && !hasAccessToDocument({ document, authPerson })) {
...
export const getDocuments: ({
  ...
  return documentCollection
    .find<Document>({}, { projection: { json: 0 } })
    .sort({ createdAt: -1 })
    .skip(page * 20)
    .limit(20)
    .toArray();
